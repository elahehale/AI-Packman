...
def euclideanHeuristic(position, problem, info={}):
    ...
    return math.sqrt((position[0] - problem.goal[0]) ^ 2 + (position[1] - problem.goal[1]) ^ 2)
...
def cornersHeuristic(state, problem):
    ...
    for c in corners:
        if c not in state[1]:
            remain.append(c)
    max = 0
    for corner in remain:
        m = abs(state[0][0] - corner[0]) + abs(state[0][1] - corner[1])
        if m > max:
            max = m
        sum += abs(state[0][0] - corner[0]) + abs(state[0][1] - corner[1])
    # other heuristic
    # return sum /2
...
def foodHeuristic(state, problem):
    ...
    for r in foodGrid:
        for c in r:
            if c:
                uneatenNum += 1
    # first heuristic is commented bellow
    # return (max(distances) + uneatenNum) / 2
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        return curr_path
        if limit <= 100:
            new_prob = PositionSearchProblem(gameState, start=startPosition,
                                goal=curr_state, warn=False, visualize=False)
